...
try {
    ...
    if ($is_draft) {
        if ($from_draft) {
            $statement = $con->prepare('UPDATE draft_posts SET title = ?, created_date = CURRENT_DATE(), image = ?, description = ?, category = ? WHERE draft_id = ? AND user_id = ?');
            $statement->bind_param('ssssii', $title, $filename, $description, $category, $draft_id, $user_id);
        } else {
            $statement = $con->prepare('INSERT INTO draft_posts (title, created_date, image, description, category, user_id) VALUES (?, CURRENT_DATE(), ?, ?, ?, ?)');
    ...
    $statement = $con->prepare('INSERT INTO blogs (title, created_date, image, description, category, user_id) VALUES (?, CURRENT_TIMESTAMP(), ?, ?, ?, ?)');
    $statement->bind_param('ssssi', $title, $filename, $description, $category, $user_id);
    $statement->execute();
    $statement->close();
