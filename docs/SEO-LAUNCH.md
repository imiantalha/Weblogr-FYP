...
This repository contains the technical SEO foundation. Ranking is still dependent on the production domain, crawlability, useful content, links, and search-engine indexing.

## Before launch

- Set `APP_URL` to the canonical HTTPS production URL.
- Confirm `robots.txt` is reachable at `/robots.txt`.
- Confirm `/sitemap.php` returns valid XML and uses the production host.
- Confirm `/seo-links.html` is reachable and describes Muhammad Talha consistently.
- Confirm `.well-known/security.txt` is reachable.
...
3. Verify the production domain in Bing Webmaster Tools.
